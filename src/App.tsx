...
export default function App() {
  ...
  const handleWithdraw = async () => {
    ...
    if (isNaN(amount) || amount < 30) {
      setError("Minimum withdrawal is 30 points.");
      return;
    }
        ...
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-slate-gray text-sm">
              {activeTab === 'home' ? "Let's earn some points today!" : 
               activeTab === 'tasks' ? "Complete tasks for massive rewards" :
               activeTab === 'invite' ? "Earn 50 points per referral" :
               activeTab === 'wallet' ? "Cash out your hard-earned points" : "Manage your account stats"}
            </p>
          </div>
          {activeTab === 'home' && (
            ...
            >
              ...
              <div className="gradient-card rounded-3xl p-8 shadow-2xl shadow-primary/20">
                ...
                <div className="flex items-baseline gap-2">
                  <h2 className="text-6xl font-black text-white">{Math.floor(profile?.balance || 0)}</h2>
                  <span className="text-xl font-bold text-white/70">pts</span>
                </div>
              </div>
                ...
                >
                  <div className="flex items-center gap-4">
                    ...
                    <div>
                      ...
                      <p className="text-slate-gray text-xs">Claim your daily 10 points</p>
                    </div>
                  </div>
                ...
                >
                  <div className="flex items-center gap-4">
                    ...
                    <div>
                      ...
                      <p className="text-slate-gray text-xs">Earn 2 points instantly</p>
                    </div>
                  </div>
          ...
          {activeTab === 'tasks' && (
            ...
            >
              ...
              <div className="space-y-4">
                ...
                {[
                  { id: 'tg_join', title: 'Join @ebisa_emoji', reward: 50, type: 'Telegram', link: 'https://t.me/ebisa_emoji' },
                  { id: 'yt_sub', title: 'Subscribe YouTube', reward: 100, type: 'YouTube', link: '#' },
                  { id: 'tw_follow', title: 'Follow on X', reward: 30, type: 'Social', link: '#' }
                ].map((task) => (
          ...
          {activeTab === 'invite' && (
            ...
            >
              <div className="stats-card p-10 rounded-3xl text-center border-primary/20 border-2">
                ...
                <p className="text-slate-gray mb-8">Earn 50 points for every friend who starts earning with us. No limits!</p>
                
                <div className="flex gap-2">
                   <div className="flex-1 bg-white/5 p-4 rounded-xl border border-white/10 text-sm font-mono truncate">
                    t.me/JaallatamaaBot?start={user?.uid}
                   </div>
                   <button 
                    onClick={() => {
                      navigator.clipboard.writeText(`https://t.me/JaallatamaaBot?start=${user?.uid}`);
                      alert("Link copied!");
                    }}
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <div className="stats-card p-6 rounded-2xl text-center">
                  ...
                  <p className="text-3xl font-black text-primary">{Math.floor(profile?.referralEarnings || 0)}</p>
                </div>
              </div>
              ...
              <div className="flex items-center gap-3 p-4 bg-white/5 rounded-2xl border border-white/5 italic text-sm text-slate-gray">
                ...
                "Our system verifies every referral instantly. You get paid 50 points the moment they open the app."
              </div>
            </motion.div>
          )}
            ...
            >
              <div className="stats-card p-6 rounded-3xl space-y-6">
                <div className="space-y-2">
                  <label className="text-xs font-bold uppercase text-slate-gray block ml-1">Withdraw Amount (Min 30)</label>
                  <input 
                    type="number"
                    placeholder="Enter points (e.g. 50)"
                    value={withdrawAmount}
                    onChange={(e) => setWithdrawAmount(e.target.value)}
                    className="w-full bg-white/5 border border-white/10 rounded-2xl p-4 text-white font-bold placeholder:text-slate-gray/50 focus:border-primary outline-none transition-colors"
                  />
                </div>
                ...
                <div className="p-4 bg-primary/5 border border-primary/20 rounded-2xl flex items-start gap-3">
                  ...
                  <p className="text-xs font-medium text-slate-gray">
                    Withdrawals are processed within 24 hours. Ensure your wallet address is correct. {profile?.has_withdrawn ? 'Needed for next: 10' : 'Min Required: 30'}
                  </p>
                </div>

                <button 
                  onClick={handleWithdraw}
                  disabled={withdrawing || profile!.balance < 30}
                  className={`w-full py-5 rounded-2xl font-black text-xl transition-all ${
                    withdrawing || profile!.balance < 30 
                  ...
                  withdrawals.map((item) => (
                    <div key={item.id} className="stats-card p-5 rounded-2xl flex items-center justify-between">
                      <div className="flex items-center gap-4">
                        ...
                        <div>
                          <h4 className="font-bold text-sm">{item.amount} PTS</h4>
          ...
          {activeTab === 'profile' && (
            ...
            >
              <div className="stats-card p-8 rounded-3xl text-center">
                ...
                <p className="text-primary text-sm font-bold uppercase tracking-widest">Active Member</p>
              </div>

              <div className="grid grid-cols-2 gap-4">
                {[
                  { label: 'Balance', value: Math.floor(profile?.balance || 0), color: 'text-white' },
                  { label: 'Invites', value: profile?.total_invites || 0, color: 'text-white' },
                  { label: 'Total Ads', value: profile?.adsWatched || 0, color: 'text-white' },
                  { label: 'Unpaid Ads', value: profile?.adsSinceLastWithdrawal || 0, color: 'text-primary' }
                ].map((stat) => (
              ...
              <section className="space-y-4">
                <h4 className="text-slate-gray text-xs font-bold uppercase tracking-widest px-1">Social Channels</h4>
                <div className="space-y-2">
                  <a href="#" className="stats-card p-5 rounded-2xl flex items-center justify-between group">
