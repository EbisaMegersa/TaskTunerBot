        ...
        unsubscribeProfile = onSnapshot(doc(db, userDocPath), async (snapshot) => {
          if (snapshot.exists()) {
            ...
            try {
              ...
              if (inviterIdFromParam && parseInt(inviterIdFromParam) !== user.id) {
                try {
                  ...
                  if (!querySnapshot.empty) {
                    ...
                    // Reward inviter (50 pts) in batch
                    batch.update(doc(db, "users", inviterDoc.id), {
                    ...
                    // Track in sub-collection in batch
                    batch.set(doc(db, `users/${inviterDoc.id}/referrals/${user.id}`), {
                      telegramId: user.id,
              ...
              // Create profile in batch
              batch.set(doc(db, userDocPath), initialProfileData);
              
              // Commit everything at once
              await batch.commit();

            } catch (e) {
              console.error("Registration Batch Error", e);
